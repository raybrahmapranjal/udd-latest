...
export default function AdminLayoutClientShell({ children, user }: ShellProps) {
  ...
  const navigationItems = [
    ...
    { href: '/admin/about-us', name: 'About Us Content', icon: Info },
    ...
    ...(user.role === 'Super Admin' 
      ? [{ href: '/admin/manage-admins', name: 'Create Admin', icon: UserPlus }] 
      : []),
  ] as const;
  ...
  const triggerLogoutSequence = async () => {
    ...
    setToast({ message: 'Terminating admin session layer securely...', type: 'loading' });
    try {
      await supabase.auth.signOut();
      setTimeout(() => {
        setToast({ message: 'Session cleared. Redirecting...', type: 'success' });
        setTimeout(() => {
          ...
          <div className="bg-white border border-rose-100 rounded-3xl p-6 max-w-sm w-full relative z-10 shadow-2xl animate-in zoom-in-95 duration-200 space-y-5">
            <div className="flex gap-4">
              ...
              <div className="space-y-1">
                <h3 className="text-sm font-black text-zinc-900 uppercase tracking-tight">Exit Security Gateway?</h3>
                <p className="text-xs text-zinc-500 leading-relaxed">Are you sure you want to close your administration access node?</p>
              </div>
            </div>
        ...
        <div className="p-3.5 border-t border-rose-100 bg-rose-50/10">
          <div className="flex items-center gap-3 px-2 py-1.5 mb-2.5">
            <div className="relative shrink-0">
              <div className="h-8 w-8 rounded-full bg-rose-50 border border-rose-200 flex items-center justify-center font-black text-xs text-rose-600 uppercase">
                {user.email.charAt(0)}
              </div>
        ...
        <header id="dashboard_top_bar" className="h-16 bg-white border-b border-rose-100 shadow-xs flex items-center justify-between px-4 sm:px-6 relative z-30 shrink-0">
          ...
          <div className="relative" ref={dropdownRef}>
            <button onClick={() => setIsProfileDropdownOpen(!isProfileDropdownOpen)} className="flex items-center gap-2.5 px-3 py-1.5 rounded-xl hover:bg-rose-50/60 border border-transparent hover:border-rose-100 transition-all cursor-pointer">
              ...
              <div className="h-8 w-8 rounded-xl bg-rose-50 border border-rose-200/60 flex items-center justify-center text-rose-500 shadow-xs shrink-0">
            ...
            {isProfileDropdownOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white border border-rose-100 rounded-xl shadow-xl py-1.5 z-50 animate-in fade-in slide-in-from-top-2 duration-100">
                <div className="px-3.5 py-2 border-b border-zinc-100 sm:hidden">
                  <p className="text-xs font-bold text-zinc-800 truncate">{user.name}</p>
                  <p className="text-[9px] font-bold text-zinc-400 uppercase mt-0.5">{user.role}</p>
                </div>
